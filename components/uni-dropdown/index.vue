      ...
      >
        <block v-if="page.type == 'hierarchy' && page.submenu.length > 0">
          ...
          <block v-for="(sub, index) in page.submenu" :key="index">
            ...
            >
              <block
                v-for="(sub_second, second_index) in sub.submenu"
                :key="second_index"
              >
                <view
                  class="sub-menu"
                ...
                >
                  ...
                  >
                    <block
                      v-for="(sub2, sub2_index) in sub_second.submenu"
                      :key="sub2_index"
                    >
                      <text
        ...
        <block v-if="page.type == 'filter'">
          <view class="filter">
            <scroll-view class="menu-box" :scroll-y="true">
              <view
                class="box"
                v-for="(box, box_index) in page.submenu"
                :key="box_index"
              >
  ...
  methods: {
    ...
    setMenuName() {
      for (var i = 0; i < this.activeMenuArr.length; i++) {
        const row = this.activeMenuArr[i]
    ...
    //展开更多
    showMoreSub(index) {
      this.subData[this.showPage].submenu[
        this.activeMenuArr[this.showPage][0]
    ...
    selectHierarchyMenu(page_index, level1_index, level2_index, level3_index) {
      ...
      if (
        ...
        level2_index != null ||
        (level1_index != null &&
          this.subData[page_index].submenu[level1_index].submenu.length == 0)
      ) {
        const sub = this.subData[page_index].submenu[level1_index].submenu[
          level2_index
        ]
        if (this.updateMenuName) {
    ...
    resetFilterData(page_index) {
      ...
      while (level > 0) {
        tmpArr.push([])
        const box = this.subData[page_index].submenu[level - 1].submenu
        for (let i = 0; i < box.length; i++) {
          this.subData[page_index].submenu[level - 1].submenu[
            i
          ].selected = false
        }
        level--
    ...
    selectFilterLabel(page_index, box_index, label_index) {
      const find_index = this.activeMenuArr[page_index][box_index].indexOf(
        label_index
      )
      if (find_index > -1) {
        this.activeMenuArr[page_index][box_index].splice(find_index, 1)
        this.subData[page_index].submenu[box_index].submenu[
          label_index
        ].selected = false
      } else {
    ...
    selectRadioLabel(page_index, box_index, label_index) {
      const activeIndex = this.activeMenuArr[page_index][box_index][0]
      if (activeIndex == label_index) {
        ...
        if (
          activeIndex != null &&
          activeIndex <
            this.subData[page_index].submenu[box_index].submenu.length
        ) {
          this.subData[page_index].submenu[box_index].submenu[
            activeIndex
          ].selected = false
    ...
    confirm() {
      const index = JSON.parse(JSON.stringify(this.shadowActiveMenuArr))
      const value = JSON.parse(JSON.stringify(this.shadowActiveMenuArr))
    ...
    reloadActiveMenuArr() {
      for (let i = 0; i < this.filterData.length; i++) {
        let tmpitem = this.filterData[i]
        const tmpArr = this.processActive(tmpitem)
        tmpitem = this.processSubMenu(tmpitem)
    ...
    processPage(index) {
      ...
      if (this.menu[index].type == 'filter') {
        //重载筛选页选中状态
        const level = this.shadowActiveMenuArr[index].length
        for (let i = 0; i < level; i++) {
          const box = this.subData[index].submenu[i].submenu
